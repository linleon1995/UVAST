...
def mlflow_log(train_loss, valid_loss, epoch):
...
class Trainer:
    ...
    def train(self, args, device=None, trainloader=None, testloader=None, testing_dataloader=None):
        if args.use_alignment_dec and not args.inference_only:
            self.model.load_state_dict(torch.load(
                args.pretrained_model, map_location='cpu'), strict=False)
        ...
        if not self.args.inference_only:
            if args.optimizer == 'adam':
                ...
                ), lr=args.learning_rate, weight_decay=self.args.weight_decay)
            ...
            lr_scheduler = optim.lr_scheduler.StepLR(
                optimizer, step_size=self.args.step_size, gamma=self.args.gamma, verbose=True)
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.inference_only:
                self.inference(testing_dataloader,
                               testloader, epoch + 1, device)
                break
            start = time.process_time()
            epoch_loss = self.train_one_epoch(
                trainloader, optimizer, epoch, device)
    ...
    def train_one_epoch(self, trainloader, optimizer, epoch, device):
        ...
        for index, data in enumerate(trainloader):
            ...
            feat, gt, mask, seg_gt, seg_dur = data['feat'], data[
                'gt'], data['mask'], data['seg_gt'], data['seg_dur']

            # normalizing the durations
            seg_dur_norm = seg_dur / data['len_seq_seg'][0].unsqueeze(1)
            batch_input, batch_target, mask = feat.to(
                device), gt.to(device), mask.to(device)
            seg_gt_act, seg_dur_normalize = seg_gt.to(
                device), seg_dur_norm.to(device)
            ...
            predictions_framewise, pred_transcript, pred_crossattn, frames_to_segment_assignment = self.model(
                batch_input, mask_train, seg_data, no_split_data=(seg_gt_no_split, seg_dur_no_split))

            # generarting gt for attntion maps
            attn_mask_gt = torch.zeros(seg_dur_no_split.shape[0], seg_dur_no_split.shape[1], int(
                seg_dur_no_split.sum().item()))
            seg_cumsum = torch.cumsum(seg_dur_no_split, dim=1)
            for i in range(seg_dur_no_split.shape[1]):
                if i > 0:
                    attn_mask_gt[0, i, int(
                        seg_cumsum[0, i - 1].item()):int(seg_cumsum[0, i].item())] = 1
                else:
            ...
            if self.args.do_framewise_loss or self.args.do_framewise_loss_g:
                framewise_losses = self.frame_wise_loss(
                    predictions=predictions_framewise, batch_target=batch_target, mask=mask, epoch=epoch)

            if self.args.do_segwise_loss or self.args.do_segwise_loss_g:
                seg_gt_act_loss = F.pad(seg_gt_act.clone()[:, 1:], pad=(
                    0, 1), mode='constant', value=-1)
                segwise_losses = self.segment_wise_loss(
                    pred_transcript, seg_gt_act_loss, batch_input.shape[-1], epoch)

            if self.args.do_crossattention_action_loss_nll:
                attn_action_losses = self.attn_action_loss(
                    pred_crossattn, attn_mask_gt, batch_target)

            if self.args.use_alignment_dec and self.args.do_crossattention_dur_loss_ce:
                attn_duration_losses = self.attn_dur_loss(
                    frames_to_segment_assignment, attn_mask_gt_dur)

            loss = framewise_losses + segwise_losses + \
                attn_action_losses + attn_duration_losses
            ...
            if self.args.adap_clip_gradient:
                ...
                clip_gradient_value = max(np.percentile(
                    grad_history, self.args.clip_percentile), 0.1)
                if clip_gradient_value > 0:
                    torch.nn.utils.clip_grad_norm_(
                        self.model.parameters(), clip_gradient_value)

            optimizer.step()

            epoch_loss += loss.item()

        mlflow_log(epoch_loss, epoch_loss, epoch)
        torch.save(self.model.state_dict(), self.model_dir +
                   "/epoch-" + str(epoch + 1) + ".model")
        torch.save(optimizer.state_dict(), self.model_dir +
                   "/epoch-" + str(epoch + 1) + ".opt")

        # log training losses
        if self.args.do_framewise_loss or self.args.do_framewise_loss_g:
            self.frame_wise_loss.log_metrics(
                mode="train_framewise", epoch=epoch + 1)
        if self.args.do_segwise_loss or self.args.do_segwise_loss_g:
            self.segment_wise_loss.log_metrics(
                mode="segment_framewise", epoch=epoch + 1)
        if self.args.do_crossattention_action_loss_nll:
            self.attn_action_loss.log_metrics(
                mode="crossattention action", epoch=epoch + 1)
        if self.args.do_crossattention_dur_loss_ce:
            self.attn_dur_loss.log_metrics(
                mode="crossattention duration", epoch=epoch + 1)
    ...
    def inference(self, testing_dataloader, testloader, epoch, device):
        actions_dict_inv = {v: k for k,
                            v in testing_dataloader.actions_dict_call.items()}
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            if self.args.inference_only:
                if self.args.path_inference_model:
                    ...
                    path_to_model = self.args.experiment_path + pretrained_name_for_testing + \
                        '/model/' + self.args.dataset + '/split_' + \
                        str(self.args.split) + '/epoch-' + \
                        str(self.args.epoch_num_for_testing) + '.model'
                print('LOADING the model {}'.format(path_to_model))

                self.model.load_state_dict(torch.load(
                    path_to_model, map_location="cpu"), strict=True)
            ...
            for index, data in enumerate(tqdm(testloader, desc="testing epoch {}".format(epoch), leave=False)):
                ...
                batch_input, batch_target, batch_target_org, mask = feat.to(
                    device), gt.to(device), gt_org.to(device), mask.to(device)

                gt_cls_names = []
                for i in range(gt_org.shape[1]):
                    gt_cls_names.extend(
                        [actions_dict_inv[batch_target_org[:, i].item()]])

                pred_framewise, pred_transcript, pred_dur, pred_dur_AD, pred_transcript_AD = self.model(
                    batch_input, mask)

                # we only evaluate one sample at a time
                assert batch_input.shape[0] == 1

                # framewise predictions
                _, predicted_framewise = torch.max(pred_framewise[-1].data, 1)
                recog = self.convert_id_to_actions(
                    predicted_framewise, gt_org, actions_dict_inv)
                update_metrics(recog, gt_cls_names, metrics_framewise)

                # transcript predictions
                pred_seg_expanded = self.convert_segments_to_labels(
                    pred_transcript, pred_dur, feat.shape[-1])
                pred_seg_expanded = torch.clamp(
                    pred_seg_expanded, min=0, max=self.args.num_classes)
                recog_seg = self.convert_id_to_actions(
                    pred_seg_expanded, gt_org, actions_dict_inv)
                ...
                if self.args.use_alignment_dec:
                    ...
                    pred_seg_expanded_dur = torch.clamp(
                        pred_seg_expanded_dur, min=0, max=self.args.num_classes)
                    recog_seg_dur = self.convert_id_to_actions(
                        pred_seg_expanded_dur, gt_org, actions_dict_inv)
                    update_metrics(recog_seg_dur, gt_cls_names,
                                   metrics_segmentwise_dur)

                # evaluation with Viterbi
                if self.args.use_viterbi:
                    viterbi = Viterbi(
                        self.args, frame_sampling=self.args.viterbi_sample_rate, max_hypotheses=np.inf)
                    frame_probs = torch.softmax(
                        pred_framewise[-1].detach(), dim=1)[0, :, :].transpose(0, 1)
                    # refine predicted transcript: remove repeating actions
                    transcript = refine_transcript(
                        pred_transcript[0]).to(self.args.device)
                    duration_viterbi = viterbi.decode(
                        frame_probs, transcript, self.args.num_classes)
                    pred_seg_expanded_dur = self.convert_segments_to_labels(
                    ...
                    recog_seg = self.convert_id_to_actions(
                        pred_seg_expanded_dur, gt_org, actions_dict_inv)
                    update_metrics(recog_seg, gt_cls_names,
                                   metrics_seg_viterbi)
            ...
            print('---------------')
            acc, edit, f1s = metrics_framewise.print(
                mode + " framewise", epoch=epoch, print_best=['acc'])
            print('---------------')
            acc_seg, edit_seg, f1s_seg = metrics_segmentwise.print(
                mode + " segmentwise", epoch=epoch, print_best=['edit'])

            dir_name = os.path.join(self.args.results_dir, "Vanilla")
            if not os.path.isdir(dir_name):
                os.makedirs(dir_name)
            write_metrics(acc, edit, f1s, epoch,
                          os.path.join(dir_name, "metrics.md"))
            write_metrics(acc_seg, edit_seg, f1s_seg, epoch,
                          os.path.join(dir_name, "metrics_seg.md"))

            print('---------------')
            if self.args.use_alignment_dec:
                acc, edit, f1s = metrics_segmentwise_dur.print(
                    mode + " segmentwise dur", epoch=epoch, print_best=['acc', 'edit', 'f1@50'])
            ...
            if self.args.use_fifa:
                print('---------------')
                acc, edit, f1s = metrics_seg_fifa.print(
                    mode + " fifa", epoch=epoch)
            ...
            if self.args.use_viterbi:
                print('---------------')
                acc, edit, f1s = metrics_seg_viterbi.print(
                    mode + " viterbi", epoch=epoch)
    ...
    def convert_segments_to_labels(self, action, duration, num_frames):
        ...
        assert duration.sum().item(
        ) == num_frames, f"Prediction {duration.sum().item()} does not match number of frames {num_frames}."
    ...
    def convert_labels_to_segments(self, labels):
        ...
        segments.append((torch.tensor(-1, device=labels.device),
                        segments[-1][-1], segments[-1][-1]))

        target_labels = torch.stack([one_seg[0] for one_seg in segments]).unsqueeze(
            0) + 2  # two is because we are adding our sos and eos

        target_durations_unnormalized = self.compute_offsets(
            [one_seg[2] for one_seg in segments]).to(target_labels.device).unsqueeze(0)
    ...
    def compute_offsets(seldf, time_stamps):
        ...
        time_stamps_unnormalized = torch.tensor(
            [float(i - j) for i, j in zip(time_stamps[1:], time_stamps[:-1])])
        return time_stamps_unnormalized

    def convert_labels(self, labels):
        action_borders = [i for i in range(
            len(labels) - 1) if labels[i] != labels[i + 1]]
        action_borders.insert(0, -1)
        action_borders.append(len(labels) - 1)
        label_start_end = []
        for i in range(1, len(action_borders)):
            label, start, end = labels[action_borders[i]
                                       ], action_borders[i - 1] + 1, action_borders[i]

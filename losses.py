...
class FrameWiseLoss(nn.Module):
    def __init__(self, args):
        ...
        self.CELogit = nn.CrossEntropyLoss(ignore_index=-1)
    ...
    def __call__(self, predictions, batch_target, mask, epoch):
        ...
        framewise_loss_random_singleframe = torch.tensor(
            0.0, device=mask.device)

        # framewise cross-entropy loss on logits of cncoder
        if self.args.do_framewise_loss:
            for i in range(len(predictions)):
                framewise_loss = framewise_loss + \
                    self.CELogit(predictions[i], batch_target.long())
        ...
        if self.args.do_framewise_loss_g:
            for i in range(len(predictions)):
                ...
                for fcls in torch.unique(batch_target).view(-1):
                    if fcls != -1:
                        ...
                        if self.args.framewise_loss_g_apply_logsoftmax:
                            ...
                            assert (
                                self.args.framewise_loss_g_apply_nothing or self.args.framewise_loss_g_apply_logsoftmax), ' you need to define the framewise group model'
                        # computing the average
                        suming_similar_act = (
                            predictions_group*((batch_target == fcls) + 0.0).unsqueeze(1)).sum(2).sum(0, keepdim=True)
                ...
                # computing the loss
                if self.args.framewise_loss_g_apply_logsoftmax:
                    framewise_loss_g = framewise_loss_g + \
                        self.NLLLoss(g_pred, g_gt.long())
                elif self.args.framewise_loss_g_apply_nothing:
                    framewise_loss_g = framewise_loss_g + \
                        self.CELogit(g_pred, g_gt.long())
            framewise_loss_g = framewise_loss_g / len(predictions)

        # final loss
        loss = framewise_loss + framewise_loss_g

        self.update_metrics(predictions[0], batch_target, g_pred, g_gt, mask, loss,
                            framewise_loss, framewise_loss_g, framewise_loss_random_singleframe)
        return loss
    ...
    def update_metrics(self, predictions, batch_target, g_pred, g_gt, mask, loss, framewise_loss, framewise_loss_g, framewise_loss_random_singleframe):
        ...
        self.framewise_loss_random_singleframe.update(
            framewise_loss_random_singleframe.item())
    ...
    def __call__(self, pred_action, target_action, vid_len, epoch):
        ...
        if self.args.do_segwise_loss:
            for i in range(len(pred_action)):
                segwise_loss = segwise_loss + \
                    self.CELogit(pred_action[i], target_action.long())
        ...
        if self.args.do_segwise_loss_g:
            ...
            for fcls in torch.unique(target_action).view(-1):
                if fcls != -1:
                    ...
                    # this can be also in a outside loop
                    for i in range(len(pred_action)):
                        #  here we normalize the features somehow, either via norm, or signmoid
                        ...
                        # computing the average
                        suming_similar_act = (
                            predictions_group * ((target_action == fcls) + 0.0).unsqueeze(1)).sum(2).sum(0, keepdim=True)
                        numm_similar_act = (
                            (target_action == fcls) + 0.0).sum()
                        mean_fcls_feat = suming_similar_act / numm_similar_act
                        # append to list
        ...
        self.update_metrics(pred_action[-1], target_action, g_pred, g_gt,
                            loss, segwise_loss, segwise_loss_g, segwise_loss_g_indecs)
        return loss
...
class AttentionLoss(nn.Module):
        ...
        if self.args.do_crossattention_action_loss_nll:
            for i in range(len(pred_attn)):
                for batchnum in range(pred_attn[i].shape[0]):
                    valid_pred_attn_lln = pred_attn[i].permute(0, 2, 1)[batchnum].unsqueeze(
                        0)[batch_target[batchnum].repeat(self.args.n_head_dec, 1) != -1]
                    valid_target_attn_lln = (target_attn.permute(0, 2, 1)[
                                             batchnum][batch_target[batchnum] != -1]).repeat(self.args.n_head_dec, 1).to(self.args.device)
                    valid_pred_attn_log = self.LogSoftmax(
                        valid_pred_attn_lln/self.args.temperature)
                    attn_loss_tmp = attn_loss_tmp + (self.NLLLoss(valid_pred_attn_log, valid_target_attn_lln.argmax(
                        1).view(-1).long()) * float(1 / pred_attn[i].shape[0]))
            attn_loss_nll = attn_loss_tmp / (len(pred_attn) + 0.0)

        loss = attn_loss_nll

        valid_pred_attn_lln_all, valid_target_attn_lln_all = None, None
        valid_pred_attn_lln_all = pred_attn[-1].permute(
            0, 2, 1)[batch_target.repeat(self.args.n_head_dec, 1) != -1]
        valid_target_attn_lln_all = (target_attn.permute(
            0, 2, 1)[batch_target != -1]).repeat(self.args.n_head_dec, 1)

        self.update_metrics(loss, attn_loss_nll, attn_loss_ce, pred_attn,
                            target_attn, valid_pred_attn_lln_all, valid_target_attn_lln_all)
        return loss
    ...
    def update_metrics(self, loss, attn_loss_nll, attn_loss_ce, pred_attn, target_attn, valid_pred_attn_lln, valid_target_attn_lln):
        ...
        if valid_target_attn_lln is not None:
            ...
            correct2 = (predicted == valid_target_attn_lln.argmax(
                1).view(-1).long().to(self.args.device)).sum().item()
...
class DurAttnCALoss(nn.Module):
    ...
    def __call__(self, pred_attn_bmm, target_attn):
        ...
        attn_loss_segmentid_ce_inside_CA = torch.tensor(
            0.0, device=target_attn.device)
        ...
        self.update_metrics(loss, attn_loss_segmentid_ce,
                            attn_loss_segmentid_ce_inside_CA, pred_attn_bmm, target_attn)
        return loss
    ...
    def update_metrics(self, loss, attn_loss_segmentid_ce, attn_loss_segmentid_ce_inside_CA, pred_attn_bmm, target_attn):
        ...
        self.attn_loss_segmentid_ce_inside_CA.update(
            attn_loss_segmentid_ce_inside_CA.item())

        _, predicted = torch.max(pred_attn_bmm.data, 2)
        correct2 = (predicted.view(-1) ==
                    target_attn.argmax(1).view(-1).long()).sum().item()
